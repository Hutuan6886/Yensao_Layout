    ...
    return (
        <div className='flex flex-row items-center justify-start gap-2'>
            <div className='relative size-4 flex items-center'>
                ...
                <div className={`relative size-4 border ${checked ? "border-[#c58c37]" : "border-[#353333]"} rounded-[0.3rem] transition`}>
                    <FaCheck className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 size-2 text-[#c58c37]
                                        ${checked ? "opacity-100" : "opacity-0"}`} />
                </div>
            </div>
            <label htmlFor={name} style={{
                fontSize: titleSize
            }} className={`text-sm cursor-pointer ${checked ? "text-[#c58c37]" : "hover:text-zinc-500"}`}
